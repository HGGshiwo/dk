...
using TimeoutScheduler =
    std::function<std::function<void()>(uint32_t, std::function<void()>)>;
// ============================================================================
// 1. 基础组件：异常与取消令牌
// ============================================================================
...
class IAsyncRuntime {
   public:
    ...
    virtual std::function<void()> set_future_timeout(
        uint32_t ms, std::function<void()> on_timeout) = 0;
...
class CancellationToken {
   ...
   public:
    ...
    CancellationToken(std::shared_ptr<std::atomic<bool>> flag)
        : cancelled_(flag) {}
...
class CancellationTokenSource {
   ...
   public:
    ...
    void cancel_after(uint32_t ms, TimeoutScheduler timeout_scheduler) {
        ...
        timeout_scheduler(
            ms, [weak_flag = std::weak_ptr<std::atomic<bool>>(cancelled_)]() {
    ...
    void cancel_after(uint32_t ms, IAsyncRuntime* rt) {
        cancel_after(ms, [rt](uint32_t m, std::function<void()> cb) {
            return rt->set_future_timeout(m, std::move(cb));
        });
    }
};
...
namespace detail {
// 默认情况 (IsFuture = false): 传入的不是 Future<T>，原样吐出 T
template <typename T, bool IsFuture>
struct unwrap_future_type {
    using type = T;
};

// 特化情况 (IsFuture = true): 传入的是 Future<T>，剥离外层，吐出内部的
// value_type
template <typename T>
...
template <typename T>
struct SharedState {
    // 技巧：如果 T 是 void，内部存储实际上用 Unit 代替
    using StorageT = std::conditional_t<std::is_void_v<T>, Unit, T>;

    std::mutex mtx;
    std::condition_variable cv;
    std::thread::id dispatcher_thread_id;
    PromiseState state = PromiseState::PENDING;
    std::optional<StorageT> value;  // ✅ 支持 void
    std::exception_ptr error;
    std::optional<CancellationToken> token;

    std::function<void(StorageT)> on_resolve;
...
class Future {
   ...
   public:
    ...
    Future<T> timeout(uint32_t timeout_ms) {
        ...
        auto next_promise = std::make_shared<Promise<T>>(
            state_->dispatcher, state_->timeout_scheduler);
        auto next_future = next_promise->get_future();

        // 设置定时器
        auto cancel_timer =
            state_->timeout_scheduler(timeout_ms, [next_promise]() {
                next_promise->reject(
                    std::make_exception_ptr(TimeoutException()));
            });

        // 注册回调：如果原始 Future 先完成，则取消定时器并透传结果
        std::function<void(StorageT)> wrapped_resolve =
            [next_promise, cancel_timer](StorageT val) {
        ...
        std::function<void(std::exception_ptr)> wrapped_reject =
            [next_promise, cancel_timer](std::exception_ptr e) {
                if (cancel_timer) cancel_timer();
                next_promise->reject(e);
            };

        register_callbacks(std::move(wrapped_resolve),
                           std::move(wrapped_reject));
    ...
    Future<T> with_cancellation(CancellationToken new_token) {
        ...
        auto next_promise = std::make_shared<Promise<T>>(
            state_->dispatcher, state_->timeout_scheduler, new_token);
        auto next_future = next_promise->get_future();
        std::function<void(StorageT)> wrapped_resolve =
            [next_promise, new_token](StorageT val) {
                if (new_token.is_cancelled()) {
                    next_promise->reject(
                        std::make_exception_ptr(CancelledException()));
                } else {
                    if constexpr (std::is_void_v<T>) {
                        next_promise->resolve();
                    } else {
        ...
        std::function<void(std::exception_ptr)> wrapped_reject =
            [next_promise, new_token](std::exception_ptr e) {
                if (new_token.is_cancelled()) {
                    next_promise->reject(
                        std::make_exception_ptr(CancelledException()));
                } else {
    ...
    Future<T> catch_error(CatchFunc&& catch_cb) {
        ...
        std::function<void(std::exception_ptr)> wrapped_reject =
            [catch_cb = std::forward<CatchFunc>(catch_cb),
    ...
    /*
        支持四种入参：0个参数，1个参数：上一个Future的返回值 or
       CacellationToken， 2个参数：上一个Future的返回值 + CancellationToken
    */
    template <typename Func>
    auto then(Func&& cb) {
        // --- 1. 针对上一层 T 是否为 void，区别匹配函数的参数 ---
        static constexpr bool is_void_in = std::is_void_v<T>;
        static constexpr bool takes_both =
            !is_void_in && std::is_invocable_v<Func, T, dk::CancellationToken>;
        static constexpr bool takes_t =
            !is_void_in && std::is_invocable_v<Func, T>;
        static constexpr bool takes_token =
            std::is_invocable_v<Func, dk::CancellationToken>;
        static constexpr bool takes_none = std::is_invocable_v<Func>;

        static_assert(takes_both || takes_t || takes_token || takes_none,
                      "Callback signature is not supported!");

        using RawRetTrait = std::conditional_t<
            takes_both, std::invoke_result<Func, T, dk::CancellationToken>,
            std::conditional_t<
                takes_t, std::invoke_result<Func, T>,
                std::conditional_t<
                    takes_token,
                    std::invoke_result<Func, dk::CancellationToken>,
                    std::invoke_result<Func>>>>;

        using RawRetType = typename RawRetTrait::type;
        constexpr bool returns_future = is_future<RawRetType>::value;
        using InnerType =
            typename detail::unwrap_future_type<RawRetType,
                                                returns_future>::type;

        // 生成下一个 promise (即便 InnerType 为 void 也能正常实例化)
        auto next_promise = std::make_shared<Promise<InnerType>>(
            state_->dispatcher, state_->timeout_scheduler, state_->token);
        auto next_future = next_promise->get_future();

        // --- 2. 改造包装函数（入参接收 StorageT） ---
        std::function<void(StorageT)> wrapped_cb = [cb = std::forward<Func>(cb),
                                                    next_promise,
                                                    current_state =
                                                        this->state_](
                                                       StorageT val) mutable {
            try {
                if (current_state->token &&
                    current_state->token->is_cancelled()) {
                    throw CancelledException();
                }

                auto execute_cb = [&]() -> decltype(auto) {
                    if constexpr (takes_both) {
                        return cb(std::move(val), current_state->token.value_or(
                                                      dk::CancellationToken{}));
                    } else if constexpr (takes_t) {
                        return cb(std::move(val));
                    } else if constexpr (takes_token) {
                        return cb(current_state->token.value_or(
                            dk::CancellationToken{}));
                ...
                if constexpr (returns_future) {
                    ...
                    if constexpr (std::is_void_v<InnerType>) {
                        inner_future
                            .then([next_promise]() { next_promise->resolve(); })
                            .catch_error([next_promise](std::exception_ptr e) {
                                next_promise->reject(e);
                            });
                    } else {
                        inner_future
                            .then([next_promise](InnerType inner_val) {
                    ...
                    if constexpr (std::is_void_v<RawRetType>) {
                        execute_cb();
                        next_promise->resolve();  // ✅ 调用无参 resolve
                    } else {
                        next_promise->resolve(execute_cb());
                    }
                }
            } catch (...) {
                next_promise->reject(std::current_exception());
        ...
        std::function<void(std::exception_ptr)> wrapped_reject =
            [next_promise](std::exception_ptr e) { next_promise->reject(e); };
    ...
    Future<T> finally(Func cb) {
        // 1. 探测 cb 返回值类型（允许 finally 回调返回 Future
        // 来实现异步等待清理）
        ...
        // 2. 成功分支包装
        std::function<void(StorageT)> wrapped_resolve =
            [cb, next_promise](StorageT val) mutable {
                try {
                    if constexpr (returns_future) {
                        auto res_future = cb();
                        // 核心技巧：使用 shared_ptr 包装 val，解决 T 可能是
                        // Move-Only 且 std::function 要求 lambda 必须
                        // CopyConstructible 的问题
                        auto val_ptr =
                            std::make_shared<StorageT>(std::move(val));

                        res_future
                            .then([next_promise,
                                   val_ptr](InnerType /* ignore */) {
                                // 清理完成，继续透传原来的值
                                if constexpr (std::is_void_v<T>) {
                                    next_promise->resolve();
                                } else {
                        ...
                        if constexpr (std::is_void_v<T>) {
                            ...
                            next_promise->resolve(
                                std::move(val));  // 继续透传原来的值
                        }
                    }
                } catch (...) {
                    next_promise->reject(
                        std::current_exception());  // 同步抛错，覆盖原结果
                }
            };
        // 3. 失败分支包装
        std::function<void(std::exception_ptr)> wrapped_reject =
            [cb, next_promise](std::exception_ptr original_e) mutable {
                try {
                    if constexpr (returns_future) {
                        auto res_future = cb();
                        res_future
                            .then([next_promise,
                                   original_e](InnerType /* ignore */) {
                                // 清理完成后，继续抛出原来的异常
                                next_promise->reject(original_e);
                                return 0;
                            })
                            .catch_error(
                                [next_promise](std::exception_ptr new_e) {
                    ...
                    next_promise->reject(
                        std::current_exception());  // 同步抛错，覆盖旧异常
                }
            };
        register_callbacks(std::move(wrapped_resolve),
                           std::move(wrapped_reject));
        ...
        if (state_->state == PromiseState::PENDING) {
            if (state_->dispatcher_thread_id == std::this_thread::get_id()) {
                throw std::logic_error(
                    "FATAL: Do not call get() or wait() inside the async event "
                    "loop thread! It will cause a deadlock.");
            }
        }
        state_->cv.wait(
            lock, [this]() { return state_->state != PromiseState::PENDING; });
    }

    // 阻塞等待指定时间（chrono 接口）
    template <class Rep, class Period>
    PromiseState wait_for(
        const std::chrono::duration<Rep, Period>& timeout_duration) const {
        std::unique_lock<std::mutex> lock(state_->mtx);
        state_->cv.wait_for(lock, timeout_duration, [this]() {
            return state_->state != PromiseState::PENDING;
        });
        return state_->state;
    }

    // 阻塞等待指定毫秒数（快捷接口）
    PromiseState wait_for(uint32_t timeout_ms) const {
        return wait_for(std::chrono::milliseconds(timeout_ms));
    }

    // ========================================================================
    // 改造 get()：当 T 为 void 时，执行同步等待但返回 void
    // ========================================================================
    T get() {
        wait();
   ...
   private:
    void register_callbacks(std::function<void(StorageT)> on_res,
                            std::function<void(std::exception_ptr)> on_rej) {
        std::unique_lock<std::mutex> lock(state_->mtx);
        if (state_->state == PromiseState::PENDING) {
            state_->on_resolve = std::move(on_res);
            state_->on_reject = std::move(on_rej);
        } else if (state_->state == PromiseState::FULFILLED) {
            StorageT val = state_->value.value();
...
class Promise {
   ...
   public:
    ...
    ~Promise() {
        ...
        if (is_pending) {
            reject(std::make_exception_ptr(
                TraceableException("Broken Promise: Event was dropped without "
    ...
    explicit Promise(std::shared_ptr<IAsyncRuntime> rt,
                     std::optional<CancellationToken> token = std::nullopt)
        : state_(std::make_shared<SharedState<T>>()) {
        ...
        state_->timeout_scheduler =
            [weak_rt](
                uint32_t ms,
                std::function<void()> on_timeout) -> std::function<void()> {
    ...
    static Future<T> resolve(
        IAsyncRuntime* rt, U val,
        std::optional<CancellationToken> token = std::nullopt) {
    ...
    template <typename U = T, std::enable_if_t<!std::is_void_v<U>, int> = 0>
    static Future<T> resolve(
        std::shared_ptr<IAsyncRuntime> rt, U val,
        std::optional<CancellationToken> token = std::nullopt) {
        auto p = Promise<T>(rt, token);
        p.resolve(std::move(val));
        return p.get_future();
    ...
    template <typename U = T, std::enable_if_t<std::is_void_v<U>, int> = 0>
    static Future<T> reject(
        IAsyncRuntime* rt, std::string error,
        std::optional<CancellationToken> token = std::nullopt) {
        auto p = Promise<T>(rt, token);
        p.reject(std::move(error));
        return p.get_future();
    }

    // ========================================================================
    // 1. 针对非 void 类型的 resolve
    // ========================================================================
    template <typename U = T, std::enable_if_t<!std::is_void_v<U>, int> = 0>
    void resolve(U val) {
        internal_resolve(std::move(val));
    }

    // ========================================================================
    // 2. 针对 void 类型的 resolve (无参数调用)
    // ========================================================================
    template <typename U = T, std::enable_if_t<std::is_void_v<U>, int> = 0>
    void resolve() {
        internal_resolve(Unit{});
    }
   ...
   private:
    // 真正的底层解决器：统一传递 StorageT（即 void 被折叠为 Unit）
    void internal_resolve(StorageT val) {
        std::function<void(StorageT)> cb;
        {
            std::unique_lock<std::mutex> lock(state_->mtx);
            if (state_->state != PromiseState::PENDING) return;
        ...
        state_->cv.notify_all();
        if (!state_->dispatcher) {
            throw std::runtime_error("ERROR: dispatcher is null in resolve!");
        } else {
