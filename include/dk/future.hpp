...
class Future {
   ...
   public:
    ...
    Future<T> timeout(uint32_t timeout_ms) {
        ...
        std::function<void(T)> wrapped_resolve = [next_promise,
                                                  cancel_timer](T val) {
    ...
    */
    ...
    auto then(Func&& cb) {
        // =====================================================================
        // 1. 编译期无损探测 (Probing)
        // 利用 C++17 的 is_invocable_v，完美避开泛型 Lambda
        // 和重载函数的解析问题
        // =====================================================================
        static constexpr bool takes_both =
            std::is_invocable_v<Func, T, dk::CancellationToken>;
        ...
        static_assert(takes_both || takes_t || takes_token || takes_none,
                      "Callback signature is not supported! Must accept (T, "
                      "Token), (T), (Token), or ().");
        // =====================================================================
        // 2. 惰性求值推导返回值类型 (Lazy Evaluation)
        // ⚠️ 极其关键：必须使用没有 _t 的 std::invoke_result，配合
        // std::conditional_t 只有最终胜出的分支才会被
        // ::type，未胜出的分支即使参数不匹配也不会触发硬报错 (SFINAE 安全)
        // =====================================================================
        ...
        using SafeInnerType =
            std::conditional_t<std::is_void_v<InnerType>, Unit, InnerType>;

        // 创建下一个 Promise
        auto next_promise = std::make_shared<Promise<SafeInnerType>>(
            state_->dispatcher, state_->timeout_scheduler, state_->token);
        auto next_future = next_promise->get_future();
        // 5. 包装业务 Callback
        std::function<void(T)> wrapped_cb = [cb = std::forward<Func>(cb),
                                             next_promise,
                                             current_state =
                                                 this->state_](T val) mutable {
            try {
                ...
                auto on_inner_error = [local_p](std::exception_ptr e) {
                    local_p->reject(e);
                };
                if constexpr (returns_future) {
                    ...
                    if constexpr (std::is_void_v<InnerType>) {
                        ...
                        inner_future.then(on_inner_void)
                            .catch_error(on_inner_error);
                    } else {
                        ...
                        inner_future.then(on_inner_val)
                            .catch_error(on_inner_error);
                    }
                } else {
    ...
    Future<T> finally(Func cb) {
        ...
        auto next_future = next_promise->get_future();
        // 2. 成功分支包装
        std::function<void(T)> wrapped_resolve = [cb,
                                                  next_promise](T val) mutable {
            try {
                if constexpr (returns_future) {
                    auto res_future = cb();
   ...
   private:
    void register_callbacks(std::function<void(T)> on_res,
...
class Promise {
   ...
   public:
    ...
    static Future<T> resolve(
        std::shared_ptr<IAsyncRuntime> rt, T val,
        std::optional<CancellationToken> token = std::nullopt) {
        auto p = Promise<T>(rt, token);
        p.resolve(val);
        return p.get_future();
