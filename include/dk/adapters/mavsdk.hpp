...
class MavsdkAdapter : public BaseAdapter<Context, DerivedEngine> {
   ...
   public:
    MavsdkAdapter(std::shared_ptr<DerivedEngine> engine,
    ...
    VehicleType get_target_type() {
        ...
        if (telemetry_vehicle_type == mavsdk::Vehicle::GroundRover) {
            ...
            spdlog::warn(
                "Unknown vehicle type, default to Copter logic or "
                "unsupported.");
    ...
    void bind_telemetry_context(SubscribeFunc subscribe_func,
                                CallbackFunc callback) {
        auto handle =
            (telemetry_.get()->*subscribe_func)([this, callback](auto... args) {
                callback(args..., this->engine_->get_context());
            });
        subscription_handles_.push_back(std::move(handle));
    }

    template <typename SubscribeFunc, typename CallbackFunc>
    void bind_telemetry_event(SubscribeFunc subscribe_func,
                              CallbackFunc callback) {
        auto handle =
            (telemetry_.get()->*subscribe_func)([this, callback](auto... args) {
    ...
    void bind_passthrough_context(uint16_t message_id, Callback cb) {
        if (!passthrough_) {
            spdlog::error(
                "MavlinkPassthrough plugin is null, cannot bind message!");
            return;
        }

        auto handle = passthrough_->subscribe_message(
            message_id, [this, cb](const mavlink_message_t& msg) {
    ...
    void bind_passthrough_event(uint16_t message_id, Callback cb) {
        if (!passthrough_) {
            spdlog::error(
                "MavlinkPassthrough plugin is null, cannot bind event!");
            return;
        }
