   ...
   public:
    WebAdapter(std::shared_ptr<DerivedEngine> engine, unsigned short port)
        : BaseAdapter<Context, DerivedEngine>(engine),
          ...
          conn_manager_(
              std::make_shared<ConnectionManager>(engine->get_ioc())) {
        beast::error_code ec;
        // 2. 打开 Acceptor
        ...
        if (ec) {
            std::string err =
                "Acceptor set reuse_address error: " + ec.message();
            spdlog::error("[WebAdapter Error] {}", err);
            throw std::runtime_error(err);
        }
        // 4. 绑定端口
        ...
        if (ec) {
            std::string err = "Bind port " + std::to_string(port) +
                              " failed: " + ec.message();
            spdlog::error("[WebAdapter Error] {}", err);
        ...
        if (ec) {
            std::string err = "Listen on port " + std::to_string(port) +
                              " failed: " + ec.message();
            spdlog::error("[WebAdapter Error] {}", err);
            throw std::runtime_error(err);
        }
        spdlog::info("[WebAdapter Info] Successfully listening on port {}",
                     port);
        do_accept();
    }
    ...
    void register_route(
        ...
        class HttpRouteHandler : public IProtocolHandler<WebAdapter> {
            ...
            void log_requst(const std::string& data) {
                spdlog::info("receive request: method={} path={} body={}",
                             method_, path_, data.substr(0, MAX_LOG_LENGTH));
            }
            void log_result(const std::string& data) {
                spdlog::info("send request: method={} path={} body={}", method_,
                             path_, data.substr(0, MAX_LOG_LENGTH));
            };

           public:
            HttpRouteHandler(
                WebAdapter* adapter, uint32_t timeout_ms,
            ...
            void handle(std::shared_ptr<HttpSession<WebAdapter>> session,
                ...
                try {
                    ...
                    session->send_http_response(http::status::bad_request,
                                                "{\"error\":\"Bad Request\"}");
        ...
        register_handler(
            method, path,
            std::make_shared<HttpRouteHandler>(this, timeout_ms, method_str,
                                               path, post_processor));
        spdlog::info("WebAdapter register route: method={} path={}", method_str,
                     path);
    }
    ...
    void register_ws_route(const std::string& path, WsEndpoint endpoint) {
        ...
        class WsRouteHandler : public IProtocolHandler<WebAdapter> {
           ...
           public:
            WsRouteHandler(WsEndpoint endpoint)
            ...
            void handle(std::shared_ptr<HttpSession<WebAdapter>> session,
                ...
                auto ws_session = std::make_shared<WsSessionImpl>(
                    std::move(socket), endpoint_);
                ws_session->accept(std::move(req));

                // 注：此时原 HttpSession 的生命周期自然结束，不再接收后续 HTTP
                // 请求
            }
        };
        // WebSocket 升级请求一定是 GET 方法
        register_handler(http::verb::get, path,
                         std::make_shared<WsRouteHandler>(std::move(endpoint)));
    }

    // Advanced Managed WebSocket Route
    // This route automatically hooks into ConnectionManager and intercepts ACK
    // messages
    void register_managed_ws_route(
        const std::string& path,
        std::function<void(std::shared_ptr<WsConnection>, const std::string&)>
    ...
    void register_static_dir(const std::string& url_prefix,
                             const boost::filesystem::path& local_dir) {
    ...
    void register_static_dir(const std::string& url_prefix,
        ...
        if (!fs::exists(abs_dir) || !fs::is_directory(abs_dir)) {
            spdlog::warn(
                "[WebAdapter] Static dir warning: {} does not exist or is not "
                "a directory.",
                abs_dir.string());
            // 视需求决定这里是否抛出异常
        }
        static_routes_[url_prefix] = abs_dir.string();
        spdlog::info("[WebAdapter] Mount static route: {} -> {}", url_prefix,
                     abs_dir.string());
    }

    // 读取并返回一个json
    void register_file_route(
        boost::beast::http::verb method, const std::string& path,
        const boost::filesystem::path& local_file_path,
        const std::string& content_type = "application/json") {
        register_file_route(method, path, local_file_path.string(),
                            content_type);
    }
    ...
    void register_file_route(
        ...
        class SingleFileHandler : public IProtocolHandler<WebAdapter> {
           ...
           public:
            ...
            void handle(std::shared_ptr<HttpSession<WebAdapter>> session,
                ...
                if (!ifs.is_open()) {
                    spdlog::error(
                        "[WebAdapter] FileRoute failed to open file: {}",
                        file_path_);
                ...
                session->send_http_response(http::status::ok,
                                            std::move(content), content_type_);
            }
        };
        // 转换为绝对路径，防止程序运行期间 Current Working Directory
        // 发生变化导致找不到文件
        ...
        register_handler(
            method, path,
            std::make_shared<SingleFileHandler>(abs_path, content_type));

        spdlog::info("[WebAdapter] Mount file route: {} {} -> {}",
    ...
    void enable_cors(
        ...
        spdlog::info("[WebAdapter] CORS enabled globally (Origin: {})",
                     allow_origin);
    }
...
class HttpSession
    : public std::enable_shared_from_this<HttpSession<AdapterType>> {
    tcp::socket socket_;
   ...
   public:
    ...
    void send_http_response(http::status status, std::string body,
    ...
    void serve_static_file(const std::string& base_dir, std::string req_path) {
        ...
        // 【GZIP 核心逻辑】检查浏览器是否支持 gzip，并且本地是否存在对应的 .gz
        // 文件
        auto it = req_.find(http::field::accept_encoding);
        if (it != req_.end() &&
            it->value().find("gzip") != boost::beast::string_view::npos) {
        ...
        // 即使发送的是 .gz 文件，Content-Type 依然必须是原始文件类型 (如
   ...
   private:
    ...
    static bool is_valid_match(const std::string& path,
        ...
        if (prefix.back() == '/')
            return true;  // e.g., "/home/" matches "/home/assets"
        return path[prefix.length()] ==
               '/';  // e.g., "/home" matches "/home/assets"
    }
    ...
    void process_request() {
        ...
        std::string pure_path = (query_pos != std::string::npos)
        ...
        // 3. 都找不到，返回 404
        spdlog::error("[WebAdapter] {} {} not found, return 404!",
                      std::string(boost::beast::http::to_string(req_.method())),
                      pure_path);
        send_http_response(http::status::not_found,
                           "{\"error\":\"Not Found\"}");
    }
    ...
    void send_cors_preflight() {
        ...
        res_->set(http::field::access_control_allow_origin,
                  adapter_->cors_origin_);
        res_->set(http::field::access_control_allow_methods,
                  adapter_->cors_methods_);
        res_->set(http::field::access_control_allow_headers,
                  adapter_->cors_headers_);
        res_->set(http::field::access_control_max_age,
                  "86400");  // 浏览器缓存该规则 24 小时，不用每次都发 OPTIONS
